...
public class BookService {
    ...
    private final BookEntityReposiroty bookEntityReposiroty;

    @PostMapping
    public boolean post( BookEntity bookEntity ){
        // 1. JPA 이용한 영속성/DB테이블(레코드)넣기
        bookEntityReposiroty.save( bookEntity );
        return true;
    } // f end
    ...
    public List<BookEntity> get(){
        ...
        return bookEntityReposiroty.findAll();
    } // f end

    // 3. 수정
    public boolean put( BookEntity bookEntity ){
        // 3. JPA 이용한 영속된/DB테이블(레코드)조회
        BookEntity entity = bookEntityReposiroty.findById( bookEntity.get도서번호() ).orElse( null );
    ...
    public boolean delete( int 도서번호 ){
        ...
        bookEntityReposiroty.deleteById( 도서번호 );
        return true;
    } // f end
