...
public class MemberService {
    ...
    public String login( MemberDto memberDto ){
        ...
        MemberEntity memberEntity
                = memberRepository.findByMemail( memberDto.getMemail() );
        // 2. 조회된 엔티티가 없으면
    ...
    // [3] 전달받은 token 으로 token 검증하여 유효한 token 은 회워정보(dto) 반환 유효하지 않은 token null 반환
    public MemberDto info( String token ){
        ...
        // 5. 조회 성공시 조회된 엔티티를 dto로 변환하여 반환
        return memberEntity.toDto();
    } // f end

    // [4] 로그아웃 시 redis에 저장된 토큰 삭제 서비스
    ...
    public int loginCount(){ // * 로그인시 레디스에 "RECENT_LOGIN" 이름으로 key 저장
        // 레디스에 저장된 키 들 중에서 "RENECT_LOGIN:" 으로 시작하는 모든 KEY 반환
