...
public class ProductService {
    ...
    private final MemberRepository memberEntityRepository;
    private final CategoryRepository categoryEntityRepository;
    private final FileUtil fileUtil;
    private final ImgRepository imgEntityRepository;
...
//    }
    ...
    public boolean updateProduct(
        ProductDto productDto , int loginMno ){
        // JPA
        ...
        Optional<ProductEntity> productEntityOptional = productEntityRepository.findById(productDto.getPno());
            // Optinal 클래스는 null 제어 메소드 제공
        if( productEntityOptional.isEmpty() ){ return false; } // 조회한 엔티티가 없으면
        ProductEntity productEntity = productEntityOptional.get();
        ...
        if( newFile != null && newFile.isEmpty() ){ // 새로운 이미지가 존재하면
            for( MultipartFile file : newFile ){
                String saveFileName = fileUtil.fileUpload( file );
                if( saveFileName == null ) throw new RuntimeException("파일 업로드 오류발생"); // throw new 예외클래스명 // 강제 예외 발생
                // 새 이미지 처리
    ...
    // 6. 이미지 개별 삭제
    public boolean deleteProductImg( int loginMno , long ino ){
        // 1. 이미지 엔티티 조회
