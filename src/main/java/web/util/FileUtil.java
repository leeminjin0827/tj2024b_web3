...
public class FileUtil {
    ...
    public String fileUpload(MultipartFile multipartFile ){ // (1) 매개변수로 'MultipartFile' 인터페이스 타입을 받는다.
        ...
        // (*) uuid 와(+) 파일명( 파일명에 _언더바가 존재하면 -하이픈으로 모두 변경 ) , _언더바는 uuid와 파일명 구분하는 용도
    ...
    public void fileDownload(String filename , HttpServletResponse response ){
        ...
        // * 스트림 : 바이트(데이터)가 다니는 통로 , 대용량(첨부파일)에서는 무조건 바이트 전송해야 한다.
        try{
            ...
            // * HTTP 응답 객체의 헤더 속성 추가 , 브라우저에게 다운로드 형식임을 알리자.!! , 다운로드 화면은브라우저가 제공한다.
            // response.setHeader( "Content-Disposition" , "attachment;filename=다운로드에표시할파일명" );
            // * 파일명에 한글이 존재하면 깨진다. HTTP는 본래 한글을 지원안한다.(인코딩 필요)
            ...
            fout.write( bytes );
            // 3. 서블릿 출력스트림 닫기 , .close() , 권장 : ㅅ트림에서 이동한 바이트들이 사용한 메모리들을 초기화 하면 안전하다.
